...
def load_vgg_model(device):
    ...
    model.load_state_dict(
        torch.load("models/vgg16_cifar10_robust.pth", map_location=device)
    )
    model.eval()
    return model
...
def target_pgd_attack(img, target_class, model, device):
    ...
    for i in range(max_iter):
        adv_tensor = torch.clamp(
            source_tensor + modifier, tensor_min, tensor_max
        )
        ...
        modifier = (
            torch.clamp(modifier, min=-epsilon, max=epsilon)
            .detach()
            .requires_grad_(True)
        )
