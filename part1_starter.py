...
Function load_dataset takes a string data_folder as input
Input data_folder is path to the FashionMNIST dataset
Function returns two datasets (training_set and validation_set)
"""


def load_dataset(data_folder):

    # To prepare training and validation data, we need to:
    # convert the data to tensor
    # normalize the data with mean 0.5 and std 0.5
    ...
    training_set = torchvision.datasets.FashionMNIST(
        data_folder, train=True, transform=train_transform, download=True
    )
    validation_set = torchvision.datasets.FashionMNIST(
        data_folder, train=False, transform=val_transform, download=True
    )
...
class FashionCNN(nn.Module):
    """
    This class defines the model architecture of the CNN model we use to classify FashionMNIST
    """
...
def plot_lossacc(loss_list, accuracy_list):
    ...
    ax1.set_xticks(
        epochs_list, labels=[str(ep) for ep in epochs_list], fontsize=15
    )
    ...
    line2 = ax2.plot(
        epochs_list, accuracy_list, color="green", lw=4, label="Accuracy"
    )
    ax2.tick_params(axis="y", which="major", labelsize=15)

    ax1.set_title(
        "Loss and Accuracy of {} Epochs Training".format(epochs), fontsize=20
    )
    lines = line1 + line2
...
def train_model(training_set, validation_set):
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Epoch: {epoch}, Training Loss: {loss_list[-1]:.2f}, Validation Accuracy: {accuracy*100:.2f}%"
        )
...
if __name__ == "__main__":
    # This will load the MNIST dataset and save it in the current working directory
